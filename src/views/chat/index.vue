...
async function onConversation(type: number) {
  ...
  if (type === 1) { // 基于文件的 wctGPT模式
    ...
    try {
      ...
      const lastText = ''
      const fetchChatAPIOnce = async () => {
        await fetchChatAPIFile<Chat.ConversationResponse>({
          prompt: message,
          options,
          signal: controller.signal,
          onDownloadProgress: ({ event }) => {
            ...
            try {
              const data = JSON.parse(chunk)
              const retMessage = data.data.message
              updateChat(
                +uuid,
                dataSources.value.length - 1,
                {
                  dateTime: new Date().toLocaleString(),
                  text: lastText + retMessage,
                  inversion: false,
                  error: false,
                  loading: true,
        ...
        >
          <template v-if="!dataSources.length">
            <div class="flex items-center justify-center mt-4 text-center text-neutral-300">
              ...
              <span>Welcome to wctgpt, an intelligent chatbot for you~</span>
            </div>
          </template>
          <template v-else>
            <div>
